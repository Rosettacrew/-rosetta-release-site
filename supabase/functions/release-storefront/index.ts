...
Deno.serve(async(req:Request)=>{
  ...
  try{
    ...
    const {data,error}=await supabase.from("release_products")
      .select("id,slug,artist_name,artist_type,title,product_type,description,presale_price_cents,release_price_cents,currency,release_at,status,published_at,cover_art_bucket,cover_art_path,stripe_payment_link_url,is_featured,featured_at,metadata,tracks:release_tracks(track_number,title)")
    ...
    const releases=publicData.map((r:any)=>{
      ...
      const showTracklist=meta.show_tracklist!==false; // default on; Owner can turn off in admin
      const showSwipeCue=meta.show_swipe_cue!==false;
      const tracks=Array.isArray(r.tracks)?[...r.tracks]
        .filter((t:any)=>t&&t.title!=null&&Number.isFinite(Number(t.track_number)))
        .map((t:any)=>({track_number:Number(t.track_number),title:String(t.title)}))
      ...
      show_tracklist:showTracklist,
      show_swipe_cue:showSwipeCue,
      tracks:showTracklist?tracks:[]
